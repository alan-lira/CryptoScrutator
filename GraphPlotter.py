...
class GraphPlotter:
   ...
   def plot_graph(self, graph_title, dataset, y_label, y_dataset, x_label, x_dataset, x_ticks_interval, x_ticks_rotation):
      ...
      pyplot.plot(y_dataset)
      pyplot.xlabel(x_label, fontsize = self.axis_label_fontsize)
      pyplot.xticks(range(0, len(dataset.index), x_ticks_interval), x_dataset.loc[::x_ticks_interval], rotation = x_ticks_rotation)
      pyplot.show()
